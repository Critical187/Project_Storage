...
public class MandelbrotApplication extends Application implements EventHandler<ActionEvent> {
    ...
    private void startThreads() {
        ...
        pool = Executors.newFixedThreadPool(nrofTiles);
        doneSignal = new CountDownLatch(nrofTiles*nrofTiles);
        for (int i = 0; i < nrofTiles; i++) {
            for (int j = 0; j < nrofTiles; j++) {
                Tile tile = tileFactory.createTile(i, j);
                Runnable m = new Mandelbrot(tile, pixelManager, doneSignal, new BoolFunction() {
                    @Override
        ...
        pool.execute(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Logger.getLogger(MandelbrotApplication.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        });
