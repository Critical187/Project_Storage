...
import javafxmandelbrot.ThreadOverview;
...
public class MovingBallsFX extends Application {
    ...
    private ReaderWriterMonitor monitor = new ReaderWriterMonitor();
    ...
    public void start(Stage primaryStage) {
        ...
        readerPriority.addEventHandler(MouseEvent.MOUSE_CLICKED,
            ...
            @Override
            public void handle(MouseEvent event) {
                monitor.SetPriority(readerPriority.isSelected());
            }
        });
    ...
    private void checkBoxMouseClicked(MouseEvent event, int index) {
        ...
        if (cb.isSelected() && index < 5) {
            ...
            ballArray[index] = b;
            Thread t = new Thread(new BallRunnable(b, true, monitor), "Reader " + (index + 1));
            ...
            ballArray[index] = b;
            Thread t = new Thread(new BallRunnable(b, false, monitor), "Writer " + (index - 5 + 1));
            threadArray[index] = t;
            circleArray[index].setVisible(true);
            t.start();
        } else {
